...
export default function Home() {
  ...
  return (
    <div ref={containerRef} className="relative min-h-screen bg-[#FDFBF7] text-slate-900 overflow-hidden font-sans selection:bg-black selection:text-white">
        ...
        >
          ...
          <nav className="hidden md:flex gap-8 text-xs font-mono font-medium uppercase tracking-widest text-slate-500">
            {['The Shift', 'The Method', 'The Truth'].map((item) => (
              <a key={item} href="#" className="hover:text-slate-900 transition-colors relative group">
                {item}
        ...
        <section className="mt-12 lg:mt-24 grid grid-cols-1 gap-16 lg:grid-cols-12">
          ...
          >
            ...
            <p className="max-w-xl font-serif text-xl leading-relaxed text-slate-600">
              The stars have moved since Western astrology was invented 2,000 years ago.
              Veas uses the <strong>actual sky</strong> to reveal who you <span className="italic">really</span> are.
            </p>

            <div className="flex flex-col sm:flex-row gap-5 pt-4">
              <button
                onClick={() => setIsModalOpen(true)}
                className="group relative inline-flex h-14 items-center justify-center overflow-hidden rounded-full bg-slate-900 px-10 font-medium text-white transition-all hover:scale-105 hover:shadow-xl hover:shadow-slate-900/10"
              >
                ...
                <span className="relative font-mono text-xs uppercase tracking-widest">Realign My Chart</span>
              </button>
            </div>
          </motion.div>
            ...
            <div className="relative z-10 w-full aspect-[3/4] rounded-t-[10rem] rounded-b-[2rem] border border-white/50 bg-white/40 backdrop-blur-md shadow-2xl shadow-slate-200/50 p-8 flex flex-col justify-between overflow-hidden">
              ...
              <div className="space-y-6 relative z-10">
                <div className="flex justify-between items-end border-b border-slate-900/10 pb-4">
                  <span className="font-mono text-[10px] uppercase text-slate-400">Current Position</span>
                  <span className="font-serif text-3xl text-slate-900 italic">23° 04'</span>
                </div>

                <div className="space-y-4">
                  {[
                    { label: "Your Known Sun", val: "Leo ♌", status: "Illusion", color: "text-slate-400 line-through" },
                    { label: "Actual Sky Sun", val: "Cancer ♋", status: "Truth", color: "text-indigo-600 font-semibold" }
                  ].map((row, i) => (
                    <div key={i} className="flex items-center justify-between p-4 rounded-xl bg-white/60 shadow-sm border border-white">
                      <span className="text-xs font-mono uppercase text-slate-500">{row.label}</span>
                      <span className={`font-serif text-lg ${row.color}`}>{row.val}</span>
                    </div>
                  ))}
                </div>
              </div>
